...
bool has_suffix(const std::ifstream &str, const std::string &suffix);
bool validFile(int argc, char** argv, std::ifstream &readFromFile);
...
bool validFile(int argc, char** argv, std::ifstream &readFromFile) //Make sure it's a .txt file and not error because of incorect input file type.
{
        if (argc > 1) {
                readFromFile = argv[1];
        }
